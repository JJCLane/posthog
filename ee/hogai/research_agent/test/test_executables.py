...
from ee.hogai.research_agent.executables import SWITCH_TO_RESEARCH_MODE_PROMPT, ResearchAgentExecutable
...
class TestResearchAgentExecutable(ClickhouseTestMixin, BaseTest):
    ...
    async def test_arun_sets_supermode_and_agent_mode(self):
        ...
        with patch.object(AgentExecutable, "arun", new=mock_parent_arun):
            ...
            self.assertEqual(captured_state.agent_mode, AgentMode.PRODUCT_ANALYTICS)
...
class TestResearchAgentToolsExecutable(ClickhouseTestMixin, BaseTest):
    async def test_arun_switches_to_supermode_when_agent_mode_is_research(self):
        state = AssistantState(
            messages=[
                AssistantMessage(
        ...
        # Mock the parent class arun to return a state with RESEARCH agent_mode
        mock_result = PartialAssistantState(
            messages=[AssistantToolCallMessage(content="Switched to research", tool_call_id="xyz")],
            agent_mode=AgentMode.RESEARCH,
        )

        with patch.object(AgentToolsExecutable, "arun", new_callable=AsyncMock, return_value=mock_result):
            ...
            self.assertEqual(result.messages[-1].content, SWITCH_TO_RESEARCH_MODE_PROMPT)

    async def test_arun_passes_through_when_not_supermode(self):
        state = AssistantState(
            messages=[
                AssistantMessage(
                    content="Reading taxonomy",
                    id="msg-1",
                    tool_calls=[
                        AssistantToolCall(
                            id="xyz",
        ...
        mock_result = PartialAssistantState(
            messages=[AssistantToolCallMessage(content="Events found", tool_call_id="xyz")],
        ...
        with patch.object(AgentToolsExecutable, "arun", new_callable=AsyncMock, return_value=mock_result):
            ...
            self.assertIsInstance(result, PartialAssistantState)
            self.assertEqual(result.agent_mode, AgentMode.PRODUCT_ANALYTICS)
            self.assertEqual(result.messages[-1].content, "Events found")
    ...
    def test_router_returns_root_when_no_notebook_tool_call(self):
        ...
        # The router will raise StopIteration when CREATE_NOTEBOOK tool call is not found
        # which means it should return "root" (the default behavior)
        try:
            result = node.router(state)
            self.assertEqual(result, "root")
        except StopIteration:
            # This is expected when there's no CREATE_NOTEBOOK tool call
            # The code has a bug where it uses next() without a default value
            pass
